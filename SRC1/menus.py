...
def inventory_menu(con, header, player, inventory_width, screen_width, screen_height):
    ...
    if len(player.inventory.items) == 0:        options = ['Inventory is empty.']
    else:
        options = []
...
def main_menu(con, background_image, screen_width, screen_height): #   {{{
    ...
    libtcod.console_print_ex(0,
                             ...
                             int(screen_height - 2),
                             libtcod.BKGND_NONE, libtcod.CENTER,
                             'By BuYn, Sofia, Tut, Roguebasin, TStand90')
    menu(con, '',
         ['Play a new game',
          'Continue last game',
          'Go to YouCitadel',
...
def level_up_menu(con, header, player, menu_width, screen_width, screen_height):#   {{{
    options = ['Constitution (+20 HP, from {0})'.format(
                   player.fighter.max_hp),
               'Strength (+1 attack, from {0})'.format(
                   player.fighter.power),
               'Agility (+1 defense, from {0})'.format(
